...
import { FCPVersion, FCPFlags, FCPPayload, FCPUnpackResult, hasFlag } from "./types";
...
function pad(text: string): string  { return text.padEnd(V4_PAD_SIZE, "\0"); }
function unpad(text: string): string { return text.replace(/\0+$/, ""); }
...
export class FlexCipher {
  ...
  static async packMessage(opts: PackOptions): Promise<string> {
    ...
    // flags parameter আর নেই — timestamp একাই unique
    ...
    return [version, flags, conditions, chatId, timestamp, ivB64, ctB64, signature].join(":");
  }
  ...
  static async unpackMessage(opts: UnpackOptions): Promise<FCPUnpackResult> {
    ...
    const [versionStr, flagsStr, conditions, chatId, tsStr, ivB64, ctB64, signature] = parts;
    ...
    // 2. Signature verify — এটা আগে হওয়া জরুরি
    const valid = await hmacVerify(signingKey, timestamp, ivB64, ctB64, signature);
    if (!valid)
      throw new Error("⚠️ Signature invalid — message tampered or wrong key");

    // 3. Message key — flags ছাড়া, শুধু timestamp
    const msgKey = await SessionManager.deriveMessageKey(chatKey, timestamp);
    ...
    if (hasFlag(flags, FCPFlags.DECOY) && duressPin && payload._decoy) {
      try {
        ...
        const decoyBuffer = await subtle().decrypt(
          { name: "AES-GCM", iv: b64d(payload._decoy.iv) as unknown as BufferSource },
          dk,
          b64d(payload._decoy.ct) as unknown as BufferSource
        );
        payload = JSON.parse(new TextDecoder().decode(decoyBuffer));
      } catch {
        // Wrong duress pin → real payload shown (plausible deniability intact)
      }
    }
    ...
    for (const clause of condStr.split(";")) {
      if (clause.startsWith("TIME")) {
        ...
        if (Date.now() < unlockAt)
          throw new Error(`⏱ Time-locked until ${new Date(unlockAt).toLocaleString()}`);
      }
      ...
      if (clause.startsWith("GEO")) {
        ...
        if (!ok) throw new Error("📍 GEO lock: you are not within the allowed location");
      }
    }
  }
  ...
  private static _geoCheck(targetLat: number, targetLng: number, radiusKm: number): Promise<boolean> {
    return new Promise((resolve) => {
      if (!navigator.geolocation) return resolve(false);
      navigator.geolocation.getCurrentPosition(
        ({ coords }) => {
          const dist = FlexCipher._haversine(coords.latitude, coords.longitude, targetLat, targetLng);
  ...
  static conditions = {
    none: ()                                  => "NONE",
    time: (futureDate: Date | number)         => `TIME(${futureDate instanceof Date ? futureDate.getTime() : futureDate})`,
